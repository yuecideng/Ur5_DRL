...
class Ur5(object):
    ...
    def reset(self):
        ...
        rand_x, rand_y, rand_z= np.random.uniform(-0.1,0), np.random.uniform(-0.1,0), np.random.uniform(-0.6,-0.4)
    ...
    def get_reward(self,pos,action):
        ...
        #regularization term
        r_a = np.sqrt(np.dot(action,action))
